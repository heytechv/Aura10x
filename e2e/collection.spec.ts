...
test.describe("Collection Management", () => {
  ...
  test("should allow user to add perfume to collection", async ({ page }) => {
    ...
    const loginResponsePromise = page.waitForResponse((response) => response.url().includes("/api/auth/signin"));
    ...
    const searchResponsePromise = page.waitForResponse(
      (response) => response.url().includes(`/api/perfumes?q=${searchQuery}`) && response.status() === 200
    );
